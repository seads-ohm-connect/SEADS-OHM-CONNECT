    ...
    async getSeadsData(userID) {
      ...
      return d3.json(seadsURL).then((liveData) => {
        return this.findID(userID).then( (id) => {
          ...
          return this.findIndex(this.userDeviceId, liveData).then( (ind) => {
              ...
              return this.findPower(this.userDeviceIndex, liveData).then((powTime) => {
                if(powTime){
